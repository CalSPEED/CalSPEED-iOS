...
- (NSArray *)hostIPAddresses {
    ...
    if (host) {
        if(logToConsole) NMSSHLogVerbose(@"Start %@ resolution", address);

        if (CFHostStartInfoResolution(host, kCFHostAddresses, &error)) {
            addresses = (__bridge NSArray *)(CFHostGetAddressing(host, NULL));
        }
        else {
            if(logToConsole) NMSSHLogError(@"Unable to resolve host %@", address);
        }

        CFRelease(host);
    }
    else {
        if(logToConsole) NMSSHLogError(@"Error allocating CFHost for %@", address);
    }
...
- (BOOL)connectWithTimeout:(NSNumber *)timeout {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if(logToConsole) NMSSHLogVerbose(@"libssh2 (v%s) initialized", libssh2_version(0));
    });
    ...
    while (addresses && ++index < [addresses count] && error) {
        ...
        if ([addressData length] == sizeof(struct sockaddr_in)) {
            ...
            if(logToConsole) NMSSHLogVerbose(@"Unknown address, it's not IPv4 or IPv6!");
            continue;
        }
        ...
        if (error) {
            if(logToConsole) NMSSHLogVerbose(@"Socket connection to %@ on port %ld failed with reason %li, trying next address...", ipAddress, (long)port, error);
        }
        else {
            if(logToConsole) NMSSHLogInfo(@"Socket connection to %@ on port %ld succesful", ipAddress, (long)port);
        }
    }
    ...
    if(logToConsole) NMSSHLogVerbose(@"Remote host banner is %@", [self remoteBanner]);

    // Get the fingerprint of the host
    NSString *fingerprint = [self fingerprint:self.fingerprintHash];
    if(logToConsole) NMSSHLogInfo(@"The host's fingerprint is %@", fingerprint);
    ...
    if(logToConsole) NMSSHLogVerbose(@"SSH session started");
...
- (void)disconnect {
    ...
    libssh2_exit();
    if(logToConsole) NMSSHLogVerbose(@"Disconnected");
    [self setConnected:NO];
}
...
- (BOOL)authenticateByPassword:(NSString *)password {
    ...
    if(logToConsole) NMSSHLogVerbose(@"Password authentication succeeded.");
...
- (NSArray *)supportedAuthenticationMethods {
    ...
    if (userauthlist == NULL){
        if(logToConsole) NMSSHLogInfo(@"Failed to get authentication method for host %@:%@", self.host, self.port);
        return nil;
    }

    NSString *authList = [NSString stringWithCString:userauthlist encoding:NSUTF8StringEncoding];
    if(logToConsole) NMSSHLogVerbose(@"User auth list: %@", authList);
